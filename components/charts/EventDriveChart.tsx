...
export default function EventDriveChart({ roundStats, players, eventId }: Props) {
  const data = roundStats
    .filter((s) => s.eventId === eventId && s.longestDrive > 0)
    .map((s) => {
      const p = players.find((pl) => pl.name === s.playerName);
      return { name: s.playerName, dist: s.longestDrive, team: p?.team ?? 1 };
    })
  ...
  const W = 600, H = 260;
  const pad = { top: 60, right: 16, bottom: 50, left: 48 };
  const chartW = W - pad.left - pad.right;
  const chartH = H - pad.top - pad.bottom;

  const minVal = Math.min(...data.map((d) => d.dist));
  const maxVal = Math.max(...data.map((d) => d.dist));
  const yBase = Math.floor(minVal / 20) * 20 - 10;
  const ticks = niceScale(yBase, maxVal, 4);
        ...
        {top3.map((d, i) => (
          <div key={d.name} style={{ textAlign: "center" }}>
            <div style={{
              ...
              fontFamily: "var(--font-mono)", fontSize: i === 0 ? "12px" : "10px", fontWeight: 700,
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
        <g transform={`translate(${pad.left},${pad.top})`}>
          ...
          {data.map((d, i) => {
            ...
            const bH = chartH - y(d.dist);
            return (
              <g key={d.name}>
                <rect x={x} y={y(d.dist)} width={barW} height={bH} fill={barColor} fillOpacity={0.8} rx={2} />
                <text x={x + barW/2} y={y(d.dist) - 3} fill={barColor} fontSize={8} fontFamily="Space Mono" fontWeight={700} textAnchor="middle">{d.dist}</text>
