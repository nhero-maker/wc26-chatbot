...
export default function EventSharpShooterChart({ roundStats, players, eventId }: Props) {
  const data = roundStats
    .filter((s) => s.eventId === eventId)
    .map((s) => {
      const p = players.find((pl) => pl.name === s.playerName);
      return { name: s.playerName, dev: s.sharpShooter, team: p?.team ?? 1 };
    })
    .sort((a, b) => a.dev - b.dev); // ascending (lower = better)
        ...
        {top3.map((d, i) => (
          <div key={d.name} style={{ textAlign: "center" }}>
            <div style={{
              ...
              fontFamily: "var(--font-mono)", fontSize: i === 0 ? "13px" : "10px", fontWeight: 700,
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: "block" }}>
        <g transform={`translate(${pad.left},${pad.top})`}>
          ...
          {data.map((d, i) => {
            ...
            const bH = chartH - y(d.dev);
            return (
              <g key={d.name}>
                <rect x={x} y={y(d.dev)} width={barW} height={Math.max(bH, 2)} fill={barColor} fillOpacity={0.8} rx={2} />
                <text x={x + barW/2} y={y(d.dev) - 3} fill={barColor} fontSize={8} fontFamily="Space Mono" fontWeight={700} textAnchor="middle">{d.dev}m</text>
